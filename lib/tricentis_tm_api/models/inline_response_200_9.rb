  ...
  class InlineResponse2009
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TricentisTmApi::InlineResponse2009` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TricentisTmApi::InlineResponse2009`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [precondition, status, name, description, priority, test_category, test_type, assignee, executed_by, planned_start_date, planned_end_date, automation, components, labels, cycle_name, steps, fields, id, test_case_key, key, created, updated, created_by, updated_by, cycle].hash
    end
